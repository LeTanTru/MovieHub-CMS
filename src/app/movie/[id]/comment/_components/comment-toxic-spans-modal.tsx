...
type CommentToxicSpansModalProps = {
  opened: boolean;
  comment: CommentResType;
  onClose: () => void;
};

const defaultValues: CommentToxicSpansBodyType = {
  id: '',
  toxic_spans: '[]'
};

export function CommentToxicSpansModal({
  opened,
  comment,
  onClose
  ...
  const initialValues = useMemo(() => {
    const parsedToxicSpans = parseToxicSpans(comment.toxicSpans) ?? [];

    return {
      id: comment.id ?? defaultValues.id,
      toxic_spans: JSON.stringify(
        normalizeToxicSpans(parsedToxicSpans, content.length)
      )
    };
  }, [comment.id, comment.toxicSpans, content.length]);

  const invalidateCommentBranch = () => {
    if (comment.parent) {
      invalidateQueries([
  ...
  const onSubmit = (values: CommentToxicSpansBodyType) => {
    ...
    updateCommentToxicSpans(
      ...
      },
      {
        ...
        onError: (error) => {
          logger.error('[UPDATE_COMMENT_TOXIC_SPANS_ERROR]', error);
          notify.error('Cập nhật nội dung độc hại thất bại');
        }
      }
    );
    ...
    <Modal onClose={onClose} open={opened} confirmOnClose={isFormChanged}>
      ...
      <Modal.Body>
        <BaseForm
          ...
          schema={commentToxicSpansSchema}
        >
          {(form) => {
                  ...
                  <Col className='grid-c-12'>
                    ...
                    >
                      {toxicSpanSegments.length > 0
                        ...
                        : content || 'Không có nội dung bình luận'}
                    </div>
                  </Col>
                </Row>
