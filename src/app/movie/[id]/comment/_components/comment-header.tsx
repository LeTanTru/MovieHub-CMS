...
import { Mars, Pin, Venus } from 'lucide-react';
    ...
    <div className='flex items-center justify-between'>
      <div className='flex items-center gap-x-2'>
        ...
        {comment.status === COMMENT_STATUS_SHOW && (
          <span title='Bình luận đang hiển thị' className='text-emerald-500'>
      ...
      {level === 0 && canPin && (
        <Button
          ...
          className={cn('mr-2 size-5! p-0!', {
            '[&_svg]:fill-sky-600 [&_svg]:text-sky-600': comment.isPinned
          })}
          onClick={() => onPin(comment.id, !comment.isPinned)}
        >
          <Pin
            className={cn('size-5 transition-all duration-200 ease-linear', {
